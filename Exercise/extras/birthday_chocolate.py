...
squares has an integer on it. She decides to share a contiguous(Touching the same boarder) segment
of the bar selected such that the length of the segment matches Ron's birth month and the sum of the
...
def birthday(s, d, m):
    ...
    for i in range(len(s)-m+1): # len(s)-m+1 -> handles the case where len(s)=1
        sum = 0
        for j in range(m):
            sum += s[i+j]
        if sum == d:
